...
# GPU vs vLLM GPU
output_file="$output_dir/test_qaic_sampler_top_ks__test_gpu_vs_vllm_gpu.txt"
rm $output_file

for batch_size in "${batch_sizes[@]}"; do
    for vocab_size in "${vocab_sizes[@]}"; do
        pytest_output=$(pytest --disable-warnings -s -v test_qaic_sampler_top_ks.py::test_gpu_vs_vllm_gpu \
