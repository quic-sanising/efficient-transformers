...
# CPU vs vLLM CPU
output_file="$output_dir/test_qaic_sampler_min_ps__test_cpu_vs_vllm_cpu.txt"
rm $output_file

for batch_size in "${batch_sizes[@]}"; do
    for vocab_size in "${vocab_sizes[@]}"; do
        pytest_output=$(pytest --disable-warnings -s -v test_qaic_sampler_min_ps.py::test_cpu_vs_vllm_cpu \
