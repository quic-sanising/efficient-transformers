...
# GPU vs QAIC
output_file="$output_dir/test_qaic_sampler_penalties__test_gpu_vs_qaic.txt"
rm $output_file
    ...
    for batch_size in "${batch_sizes[@]}"; do
        for vocab_size in "${vocab_sizes[@]}"; do
            for ctx_length in "${ctx_lengths[@]}"; do
                pytest_output=$(pytest --disable-warnings -s -v test_qaic_sampler_penalties.py::test_gpu_vs_qaic \
