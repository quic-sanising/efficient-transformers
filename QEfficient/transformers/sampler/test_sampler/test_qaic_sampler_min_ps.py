...
from QEfficient.transformers.sampler.test_sampler.sampler_min_ps import sampler_forward
from QEfficient.transformers.sampler.test_sampler.utils import print_difference_in_tensors
from QEfficient.transformers.sampler.test_sampler.vllm_sampler_min_ps import (
    Sampler,
    SamplingMetadata,
)


def test_cpu_vs_vllm_cpu(setup_data_min_ps):
    print(setup_data_min_ps["seed"])

    logits = setup_data_min_ps["logits"]
    vllm_logits = deepcopy(setup_data_min_ps["logits"]).squeeze(1)

    top_ks = setup_data_min_ps["top_ks"]
    top_ps = setup_data_min_ps["top_ps"]
    min_ps = setup_data_min_ps["min_ps"]
...
def test_cpu_vs_qaic(setup_data_min_ps):
    print(setup_data_min_ps["seed"])

    logits = setup_data_min_ps["logits"]
    print("Input Logits", logits)
    qaic_logits = deepcopy(setup_data_min_ps["logits"])
    print("QAIC Input Logits", qaic_logits)

    top_ks = setup_data_min_ps["top_ks"]
    qaic_top_ks = deepcopy(setup_data_min_ps["top_ks"])

    top_ps = setup_data_min_ps["top_ps"]
    qaic_top_ps = deepcopy(setup_data_min_ps["top_ps"])
...
def test_gpu_vs_qaic(setup_data_min_ps):
    print(setup_data_min_ps["seed"])

    logits = setup_data_min_ps["logits"].cuda()
    print("Input Logits", logits)
    qaic_logits = deepcopy(setup_data_min_ps["logits"])
    print("QAIC Input Logits", qaic_logits)

    top_ks = setup_data_min_ps["top_ks"].cuda()
    qaic_top_ks = deepcopy(setup_data_min_ps["top_ks"])

    top_ps = setup_data_min_ps["top_ps"].cuda()
    qaic_top_ps = deepcopy(setup_data_min_ps["top_ps"])

    min_ps = setup_data_min_ps["min_ps"].cuda()
    qaic_min_ps = deepcopy(setup_data_min_ps["min_ps"])
...
def test_gpu_vs_vllm_gpu(setup_data_min_ps):
    print(setup_data_min_ps["seed"])

    logits = setup_data_min_ps["logits"].cuda()
    vllm_logits = deepcopy(setup_data_min_ps["logits"]).squeeze(1).cuda()
