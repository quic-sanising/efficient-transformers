...
class QEffCausalLMOutputWithPast(ModelOutput):
    ...
    past_penalty_buffer: Optional[torch.Tensor] = None
...
def sampler_forward(
    ...
    past_repetition_penalty_buffer = past_penalty_buffer[:, 0:1, :]  # (full_batch_size, 1, vocab_size)
    past_presence_penalty_buffer = past_penalty_buffer[:, 1:2, :]  # (full_batch_size, 1, vocab_size)
    # --- Prefill ---
    if input_ids.shape[1] > spec_length:
        if (position_ids[:, 0] == 0).any():
            # First input chunk, so initialize retain states
            mul_value = torch.ones(past_repetition_penalty_buffer.shape[0], 1, 1, dtype=torch.bool)
            mul_value[batch_index_reshaped] = position_ids[:, :1] != 0
            past_penalty_buffer *= mul_value
        ...
        past_repetition_penalty_buffer = CtxScatterFuncCB3D.apply(
            past_repetition_penalty_buffer, batch_index, input_ids, torch.ones(input_ids.shape[0], 1, input_ids.shape[1], dtype=torch.bool))  # (full_batch_size, 1, vocab_size)
    ...
    # --- Decode ---
        ...
        scatter_values = torch.ones(
            last_accepted_output_tokens.shape[0], 1, last_accepted_output_tokens.shape[1], dtype=torch.bool)
        past_repetition_penalty_buffer = CtxScatterFuncCB3D.apply(
            past_repetition_penalty_buffer, batch_index, last_accepted_output_tokens, scatter_values)  # (full_batch_size, 1, vocab_size)
        past_presence_penalty_buffer = CtxScatterFuncCB3D.apply(
            past_presence_penalty_buffer, batch_index, last_accepted_output_tokens, scatter_values)  # (full_batch_size, 1, vocab_size)
        # TODO: For frequency retain state, first gather and then scatter
    
    past_penalty_buffer = torch.stack([past_repetition_penalty_buffer, past_presence_penalty_buffer], dim=1)  # (full_batch_size, 2, vocab_size)

    # Repetition Penalty
    if (repetition_penalties != 1.).any():
        past_repetition_penalty_buffer_selected = past_penalty_buffer[:, 0, :][batch_index_reshaped].repeat(spec_length, 1)  # (batch_size, vocab_size) -> (batch_size * spec_length, vocab_size)
    ...
    if (presence_penalties != 0.).any():
        ...
        past_presence_penalty_buffer_selected = past_penalty_buffer[:, 1, :][batch_index_reshaped].repeat(spec_length, 1)  # (batch_size, vocab_size) -> (batch_size * spec_length, vocab_size)
